...
namespace AutoGrading.Model
{
    ...
    public delegate void URHostConnectedEventHandler();

    public class URobotManager : DeviceManager
    {
        public event ImageReceivedEventHandler ImageReceivedEvent;
        public event URHostConnectedEventHandler URHostConnectedEvent;
        ...
        public URobotManager(string name, bool connecting) : base(name)
        {
        ...
        void RaiseURHostConnectedEvent()
        {
            URHostConnectedEvent?.Invoke();
        }

        public void AddURHostConnectedEventSubscriber(URHostConnectedEventHandler handler)
        {
            URHostConnectedEvent += handler;
        }
        ...
        private void TcpClientThread()
        {
            ...
            while (foreverClient)
            {
                ...
                {
                    URobotEvent evb = new URobotEvent(EventType.Error, e.Message, null, "Client");
                    DeviceEvent eb = new DeviceEvent(DeviceName, evb);
                    //EventBus.Instance.PostEvent(eb);
                    ...
                    {
                        // a blocking listen has been cancelled
                        evb = new URobotEvent(EventType.Error, "SocketError: Interrupted and Disconnected", null, "Client");
                        eb = new DeviceEvent(DeviceName, evb);
                        EventBus.Instance.PostEvent(eb);
                    }
                }
        ...
        public override bool ReconnectEx()
        {
            deviceClient = null;
            tcpURClient.Close();
            return true;
        ...
        bool SendMsgEx(string msg, bool client)
        {
            try
            {
                if (client)
                {
                    ...
                    // add newline character to the end of each command \r\n or \r or \n
                    Byte[] sendBytes = Encoding.ASCII.GetBytes(msg + "\r");
