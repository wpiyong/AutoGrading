        ...
        public static bool Connect(string interfaceName, int numRetries = 1)
        {
            ...
            for (int i = 0; i < numRetries; i++)
            {
                try
                {
                    ...
                    {
                        found = false;
                        e = new DeviceEvent("ColorMeter", new GColorEvent(EventType.Error, ex.Message, null));
                        EventBus.Instance.PostEvent(e);
                    }
                    ...
                    if (found)
                    {
                        ...
                        //e = new DeviceEvent("ColorMeter", new GColorEvent(EventType.Info, "ST5 Option Board: " + ReadLine(), null));
                        //EventBus.Instance.PostEvent(e);
                    ...
                    else
                    {
                        ...
                        e = new DeviceEvent("ColorMeter", new GColorEvent(EventType.Error, "Could not connect to Stepper Motor", null));
                        EventBus.Instance.PostEvent(e);
                    }
